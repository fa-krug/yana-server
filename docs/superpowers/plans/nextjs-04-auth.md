...
cd .. && git add yana-next && git commit -m "feat(next): Wire Better Auth with passkey and admin plugins
...
```bash
cd .. && git add yana-next && git commit -m "feat(next): Auto-create an admin when none exists
...
```bash
cd .. && git add yana-next && git commit -m "feat(next): Close the 3/4 seam with session-backed identity
...
```bash
cd .. && git add yana-next && git commit -m "feat(next): Add passkey-first login with password reveal
...
```bash
cd .. && git add yana-next && git commit -m "feat(next): Add avatars with deterministic initial fallbacks
...
```bash
cd .. && git add yana-next && git commit -m "feat(next): Add the account page and sidebar profile entry

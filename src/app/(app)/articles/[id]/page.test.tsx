 ...
 * `getArticle()` returning `null` used to throw Next's not-found sentinel
 * from an **awaited** page body, and this file used to assert exactly that
 ...
 * and a missing/non-numeric/unowned id renders the same not-found state
 * instead of throwing.
 */
...
describe("/articles/[id] page", () => {
  it("returns its element tree synchronously -- no awaited row read", () => {
    const result = ArticleDetailPage({ params: Promise.resolve({ id: "999999" }) });

    expect(result).not.toBeInstanceOf(Promise);
    expect(typeof (result as { then?: unknown })?.then).not.toBe("function");
  });

  it("renders the not-found state for an id with no article, instead of throwing", async () => {
    ...
    await act(async () => {
      renderWithProviders(ArticleDetailPage({ params: Promise.resolve({ id: "999999" }) }));
    });
  ...
  it("renders the same not-found state for a non-numeric id", async () => {
    await act(async () => {
      renderWithProviders(ArticleDetailPage({ params: Promise.resolve({ id: "not-a-number" }) }));
    });

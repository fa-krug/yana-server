...
/**
 * `FeedsPage` renders two Server Components (`FeedsBody`, `FeedsPagination`,
 ...
 * tree's shape directly, without ever handing it to `render()`.
 */
describe("FeedsPage", () => {
  it("returns its element tree synchronously -- no awaited searchParams, requireUser or translation", () => {
    ...
    // remaining await.
    const result = FeedsPage({ searchParams: Promise.resolve({}) });

    expect(result).not.toBeInstanceOf(Promise);
    expect(typeof (result as { then?: unknown })?.then).not.toBe("function");
  });

  it("renders <FeedsChrome> and <FeedsListRegion>, and shares one searchParams promise between them", () => {
    const searchParams = Promise.resolve({ q: "hello" });

    const result = FeedsPage({ searchParams }) as ReactElement;
    const children = (result.props as { children: ReactElement[] }).children;
    const [chrome, listRegion] = children;

    expect(chrome.type).toBe(FeedsChrome);
    expect(listRegion.type).toBe(FeedsListRegion);
    ...
    // downstream, `cachedListFeeds`'s) depends on. A page that awaited

...
import { ActiveRunsIndicator, ActiveRunsProvider } from "@/components/jobs/active-runs-context";
        ...
        <BreadcrumbTitleProvider>
          <ActiveRunsProvider>
            <header className="flex h-14 shrink-0 items-center gap-2 border-b px-3 md:px-4">
              <SidebarTrigger />
              {/* min-w-0 lets this shrink below its content's natural width -- a
                  flex item defaults to min-width: auto, which otherwise refuses
                  to yield space to <ActiveRunsIndicator> and forces the whole
                  header (and with it the page) wider than the viewport instead
                  of just truncating the breadcrumb. overflow-hidden is the
                  other half: shrinking alone doesn't clip the now-too-wide
                  breadcrumb content on its own. */}

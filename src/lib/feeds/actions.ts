...
import { AGGREGATOR_SPECS, schemaFor, stripUnavailable, type Capabilities } from "@/lib/aggregators/registry";
...
export async function getFeed(id: number) {
  ...
  const feed = db.select().from(feeds).where(and(eq(feeds.id, id), eq(feeds.userId, userId))).get();
  if (!feed) return null;
  
  const attachedTags = db.select({
    id: tags.id,
    name: tags.name,
    userId: tags.userId,
...
export async function listFeeds(params: ListParams) {
  ...
  if (params.filters.tag) {
    conditions.push(inArray(feeds.id, db.select({ id: feedTags.feedId }).from(feedTags).where(eq(feedTags.tagId, Number(params.filters.tag)))));
  }
  ...
  for (const row of rows) {
    ...
    const articleCountRow = db.select({ value: count() }).from(articles).where(eq(articles.feedId, row.id)).get();
...
export async function createFeed(input: FeedInput): Promise<{ ok: boolean; error?: string; field?: string; id?: number }> {
  try {
    ...
    const cleanedOptions = stripUnavailable(spec.key, optionsParsed.data as Record<string, unknown>, capabilities);
    
    const userId = await currentUserId();
    
    return writeTransaction((tx) => {
      if (tagIds.length > 0) {
        const validTags = tx.select({ id: tags.id }).from(tags).where(and(inArray(tags.id, tagIds), eq(tags.userId, userId))).all();
        if (validTags.length !== tagIds.length) {
          return { ok: false, error: "Invalid tags" };
        }
      }
      
      const feed = tx.insert(feeds).values({
        name: name as string,
        aggregator: spec.key,
        identifier,
        options: cleanedOptions,
        userId
      }).returning({ id: feeds.id }).get();
      
      if (tagIds.length > 0) {
        tx.insert(feedTags).values(tagIds.map((tagId: number) => ({
          feedId: feed.id,
          tagId
        }))).run();
      }
...
export async function updateFeed(id: number, input: FeedInput) {
  ...
  return writeTransaction((tx) => {
    ...
    }).where(and(eq(feeds.id, id), eq(feeds.userId, userId))).run();
...
export async function deleteFeeds(ids: number[]) {
  ...
  return writeTransaction((tx) => {
    const result = tx.delete(feeds).where(and(inArray(feeds.id, ids), eq(feeds.userId, userId))).run();
    revalidatePath("/feeds");
...
export async function refreshLogos(ids: number[]): Promise<{ ok: boolean; enqueued: number }> {
  ...
  return writeTransaction((tx) => {
    const validFeeds = tx.select({ id: feeds.id }).from(feeds).where(and(inArray(feeds.id, ids), eq(feeds.userId, userId))).all();
    
    if (validFeeds.length > 0) {
      tx.insert(jobs).values(validFeeds.map(f => ({
        kind: "feed.logo",
        payload: { feedId: f.id }
      }))).run();
    }
